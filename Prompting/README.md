...
| [`rag.py`](rag.py) | Retrieval-augmented generation | Write `fetch_user_name()` from API docs | Generated code contains every required snippet 📚 |
...
**RAG in two halves.** `load_corpus_from_files()` reads the docs, then `YOUR_CONTEXT_PROVIDER(corpus)` picks
which ones to ship. The selection is injected into the user message as a `Context:` block. 📚
...
| [`report.pdf`](report.pdf) | The written report for this assignment 📄 |

## 💭 Notes & takeaways

*(Written while building it.)*

- ⚠️ **`rag.py` is currently a no-retrieval baseline.** It looks for `data/api_docs.txt`, which isn't in the
  repo, so `load_corpus_from_files()` records `[missing_file]`, the provider filters that out, and the prompt
  literally says `(no context provided)`. Drop a docs file there to make it a real RAG run. 📁
