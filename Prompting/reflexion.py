...
You are a senior Python developer tasked with fixing a password validation function.
Follow these rules STRICTLY:
1. Output ONLY a single fenced Python code block with the fixed `is_valid_password(password: str) -> bool` function (no comments, no prose).
2. Fix the function to pass all test cases by enforcing these password rules:
   - Minimum length of 8 characters (implied by test cases)
   - Must contain at least one uppercase letter (A-Z)
   - Must contain at least one lowercase letter (a-z)
   - Must contain at least one digit (0-9)
   - Must contain at least one special character from this set: !@#$%^&*()-_
   - No whitespace characters allowed
3. Keep the implementation minimal and efficient.
4. Use the failure diagnostics provided to identify exactly which rules are being violated for each test case.
"""
...
def your_build_reflexion_context(prev_code: str, failures: List[str]) -> str:
    ...
    return f"""
Your previous implementation of is_valid_password failed these test cases:
{failure_text}
...
{prev_code}
```
Fix the function to resolve all these failures. Output ONLY the fixed fenced Python code block (no explanations).
The valid password rules are:
- Must have at least one uppercase letter (A-Z)
- Must have at least one lowercase letter (a-z)
- Must have at least one digit (0-9)
- Must have at least one special character from !@#$%^&*()-_
...
def run_reflexion_flow(
    ...
    improved_code = apply_reflexion(
        reflexion_prompt, build_context, initial_code, failures
    )
    print("\nImproved code:\n" + improved_code)
...
if __name__ == "__main__":
    run_reflexion_flow(
        SYSTEM_PROMPT, YOUR_REFLEXION_PROMPT, your_build_reflexion_context
    )
